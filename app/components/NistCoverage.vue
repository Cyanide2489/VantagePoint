...
import { levelFor } from "~/composables/useScoring";

defineProps<{ coverage: any[] }>();
const { t } = useI18n();
...
<template>
  <div class="space-y-3">
    <details v-for="fn in coverage" :key="fn.name" class="card overflow-hidden">
      <summary class="flex cursor-pointer list-none items-center gap-3 p-4 hover:bg-stone-50">
        ...
        <div class="h-2.5 flex-1 overflow-hidden rounded-full bg-stone-100">
          <div class="h-full rounded-full transition-all" :class="barColor(fn.score)"
      ...
      <div class="border-t border-stone-100 bg-stone-50/60 px-4 py-2">
        <div v-for="sub in fn.subcategories" :key="sub.code"
          ...
          <span class="min-w-0 flex-1 truncate text-stone-600" :title="t(sub.subcategory)">{{ t(sub.subcategory) }}</span>
