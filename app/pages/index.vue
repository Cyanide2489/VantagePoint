...
<template>
  <div class="space-y-8">
    <section class="card overflow-hidden">
      <div class="bg-brand-100 px-6 py-10 text-stone-900 sm:px-10">
        ...
        <div class="mt-6 flex flex-wrap gap-3">
          ...
          <NuxtLink
            ...
            to="/results"
            class="btn border border-brand-300 bg-white/40 text-brand-900 hover:bg-white/70"
          >
            查看結果
          </NuxtLink>
      ...
      <div
        v-if="store.answeredCount > 0"
        class="px-6 py-4 sm:px-10"
      >
        <ProgressBar
          :value="progress"
          :label="`已完成 ${store.answeredCount} / ${totalQuestions} 題`"
        />
      </div>
    </section>

    <section>
      <h2 class="mb-3 text-lg font-semibold text-stone-800">
        評估構面（共 {{ meta.questionsCount }} 題）
      </h2>
