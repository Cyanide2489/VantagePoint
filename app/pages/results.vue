...
<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:gap-8">
        <div class="grid h-32 w-32 shrink-0 place-items-center rounded-full border-8 border-brand-200 bg-brand-50">
          <div class="text-center">
            <div class="text-3xl font-bold text-brand-800">{{ overallScore.toFixed(1) }}</div>
            <div class="text-xs text-stone-500">/ 5.0</div>
          </div>
        </div>
        <div>
          <div class="text-sm text-stone-500">整體成熟度等級</div>
          <div class="text-2xl font-bold text-stone-900">L{{ Math.round(overallScore) }} · {{ overallLevel.name }}</div>
          <p class="mt-1 max-w-xl text-sm text-stone-600">{{ overallLevel.desc }}</p>
    ...
    <div class="space-y-6">
      <section class="card p-5">
        <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h2 class="font-semibold text-stone-800">成熟度雷達圖</h2>
        ...
        <MaturityRadar :labels="radarLabels" :scores="radarScores" :dense="radarMode === 'detailed'" />
        <p v-if="radarMode === 'detailed'" class="mt-2 text-xs text-stone-500">
          依各構面的項目（題組）平均成熟度顯示，共 {{ sectionCount }} 個項目。
        </p>
      </section>

      <section class="card p-5">
        <h2 class="mb-3 font-semibold text-stone-800">各構面成熟度</h2>
        <div class="space-y-4">
          <div v-for="d in domainScores" :key="d.letter">
            ...
            <div class="mt-1 h-2 w-full overflow-hidden rounded-full bg-stone-100">
              <div class="h-full rounded-full bg-brand-500" :style="{ width: pct(d.score) + '%' }" />
            </div>
          </div>
        </div>
      </section>
    ...
    <!-- NIST coverage -->
    <section class="card p-5">
      <h2 class="mb-3 font-semibold text-stone-800">NIST CSF 2.0 成熟度雷達圖</h2>
      <MaturityRadar :labels="nistRadarLabels" :scores="nistRadarScores" />
    ...
    <section>
      <h2 class="mb-3 font-semibold text-stone-800">NIST CSF 2.0 對應覆蓋度</h2>
      <p class="mb-3 text-sm text-stone-500">依據各 NIST 功能類別所對應題目的平均成熟度估算，點開可檢視子類別。</p>
