  ...
  <div class="mx-auto max-w-2xl space-y-6">
      ...
      >
        ...
        >
          <span
            ...
            :class="i <= step ? 'bg-brand-200 text-brand-900' : 'bg-stone-200 text-stone-500'"
          >{{ i + 1 }}</span>
    ...
    >
        ...
        <li class="flex gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-stone-500">
              {{ domains.length }} 大構面各自獨立，可分次完成、隨時切換，進度會自動儲存於瀏覽器。
            </p>
          </div>
        </li>
        <li class="flex gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-stone-500">
              每題提供 1–5 級的情境描述，請選擇最貼近貴組織現況的一項，不需要精確分數。
            </p>
          </div>
        </li>
        <li class="flex gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-stone-500">
              完成後可看到各構面成熟度雷達圖、整體等級，以及對應 NIST CSF 2.0 的覆蓋度，並可匯出報告。
            </p>
          </div>
        </li>
      </ul>
      <div class="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">
        <div
          v-for="d in domains"
          :key="d.letter"
          class="rounded-none border border-stone-200 p-2 text-center"
        >
          <div class="text-xl">
            {{ icon(d.letter) }}
          </div>
    ...
    >
      ...
      <p class="mt-1 text-sm text-stone-500">
        評估以 0–5 級衡量每個面向的成熟度，分數越高代表能力越完整、越制度化。
      </p>
      <ul class="mt-4 space-y-2">
        <li
          v-for="lvl in MATURITY_LEVELS"
          :key="lvl.min"
          class="flex items-start gap-3 rounded-none border border-stone-200 p-3"
        >
